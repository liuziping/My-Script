...
def apply_pub(username,data,where):
    ...
    util.write_log('api').info("%s:success and update project_apply status %s" % (username,data['status']))
    ...
    util.write_log('api').info("%s:success and insert project_deploy status  %s"  % (username,data['status']))
...
def apply_create(auth_info, **kwargs):
    ...
    try:
        ...
        util.write_log('api').info('%s:项目申请成功' % username)
