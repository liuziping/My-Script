...
CREATE TABLE `project` (
  ...
  `p_user`  varchar(32)  DEFAULT NULL COMMENT '有权限的用户,存用户ID，多个用户用字符串链接 "1,2"',
  `p_group` varchar(32)   DEFAULT  NULL COMMENT '有权限的组,存ID，多个组用字符串链接',
...
CREATE TABLE `project_test` (
  ...
  `commit` varchar(64) NOT NULL COMMENT '推送版本号，需要通过脚本从git获取',
  `pusher` varchar(64) NOT NULL COMMENT '推送人,即登陆系统的用户',
...
CREATE TABLE `project_apply` (
  ...
  `applicant` varchar(32) NOT NULL COMMENT '申请人，即登陆用户',
