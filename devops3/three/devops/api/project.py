...
def project_create(auth_info, **kwargs):
    ...
    try:
        ...
        if not app.config['db'].if_id_exist('user',data['principal'].split(',')):     
            return json.dumps({'code':1,'errmsg':'principal not exist'})
        if data.has_key('p_uesr') and not app.config['db'].if_id_exist('user',data['p_uesr'].split(',')):     
            return json.dumps({'code':1,'errmsg':'p_user not exist'})
        if data.has_key('p_group') and not app.config['db'].if_id_exist('role',data['p_group'].split(',')):     
            return json.dumps({'code':1,'errmsg':'p_group not exist'})
        data['create_date'] = time.strftime('%Y-%m-%d')
        app.config['db'].execute_insert_sql('project', data)
        # 信息入库后，调用函数，让gitolite生效，返回结果成功，则发送邮件
        res = gitolite()
        if res['code'] == 1:
            return json.dumps(res)
        # 获取当前项目的所有人，为下面发邮件准备数据
        members,principal = utils.project_members()   
        sender = members[data['name']]
        # 发邮件
        utils.write_log('api').info("%s:create project %s scucess" %  (username,data['name']))
        return json.dumps({'code':0,'result':'create project %s scucess' % data['name']})
    except:
        utils.write_log('api').error("create project error: %s" % traceback.format_exc())
...
def project_select(auth_info,**kwargs):
    ...
    try:
        ...
        # 查询用户表，生成id2name的字典
        users=utils.getinfo('user',['id', 'name'])

        # 查询角色表，生成id2name的字典
        roles=utils.getinfo('role',['id', 'name'])

        # 管理员查询项目表，把项目表中p_uesr,p_group,principal的ID 转为name
        # 结果:[{'id':1,'name':'devops','principal':'pc','p_user':'pc,wd'....},......]
        projects = app.config['db'].get_results('project', fields)
        ...
        if '1' not in  auth_info['r_id']:  
            p = utils.user_projects(username)  # 查出用户的项目id2name{'1':'devops','2':'test'}
            projects = [pro for pro in projects for pid in p.keys() if pid==pro['id']] # 获取项目的详情 
        
        utils.write_log('api').info('%s:select project list success' % username)
        return json.dumps({'code':0,'result':projects,'count':len(projects)})
    except:
        utils.write_log('api').error("select project list error: %s"  %  traceback.format_exc())
...
def project_get(auth_info, **kwargs):
    ...
    try:
        ...
        result = app.config['db'].get_one_result('project', fields, where)
        if not result :
            return json.dumps({'code':1, 'errmsg':'result is null'})
        else:
            utils.write_log('api').info("%s:select project by id success" % username)
            return json.dumps({'code':0,'result':result})
    except:
        utils.write_log('api').error('select project by id error: %s'  % traceback.format_exc())
...
def project_update(auth_info, **kwargs):
    ...
    try:
        ...
        if not where:
            return json.dumps({'code':1, 'errmsg':'must need a condition'})
        result = app.config['db'].execute_update_sql('project', data, where)
        if not result:
            return json.dumps({'code':1, 'errmsg':'result is null'})
        res = gitolite()
        if res['code'] == 1:
            return json.dumps(res)
        utils.write_log('api').info('%s:update project success!' % username)
...
def project_delete(auth_info,**kwargs):
    ...
    try:
        ...
        if not where:
            return json.dumps({'code':1, 'errmsg':'must need a condition'})
        result = app.config['db'].execute_delete_sql('project', where)
        if not result :
            return json.dumps({'code':1, 'errmsg':'result is null'})
        utils.write_log('api').info('%s:delete project successed' % username)
        return json.dumps({'code' :0,'result':'delete project scucess'})
    except:
        utils.write_log('api'). error('delete project error: %s' %  traceback.format_exc())
...
def gitolite():
    ...
    try :
        ...
        utils.write_log('api'). error("get config error: %s" % traceback.format_exc())
