...
def userinfo(auth_info,**kwargs):
    ...
    try:
        ...
        utils.write_log('api').info(username, 'get_one_user info') 
...
def userlist(auth_info,**kwargs):
    ...
    try:
        ...
        for user in result: #查询user表中的r_id,与role表生成的字典对比，一致则将id替换为name,如，"sa,php"
            ...
            users.append(user)
        utils.write_log('api').info(username, 'get_all_users')
...
def userdelete(auth_info, **kwargs):
    ...
    try: 
        ...
        if not result:
            return json.dumps({'code':1,'errmsg':'User not exist'})
        utils.write_log('api').info(username, 'Delete user successed')
...
def passwd(auth_info):
    ...
    try:
        ...
        utils.write_log('api').info(username,'update user password success')
