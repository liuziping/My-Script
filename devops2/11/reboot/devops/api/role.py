...
def role_create(auth_info, **kwargs):
    ...
    try:
        ...
        utils.write_log('api').info(username, "create role %s scucess" %  data['name'])
        return json.dumps({'code':0,'result':'create role %s scucess' % data['name']})
    except:
        utils.write_log('api').error(username,"create role error: %s" % traceback.format_exc())
...
def role_select(auth_info,**kwargs):
    ...
    try:
        ...
        utils.write_log('api').info(username, 'select role list success')
...
def role_get(auth_info, **kwargs):
    ...
    try:
        ...
        if not result :
            ...
            utils.write_log('api').info(username, "select role by id success")
...
def role_update(auth_info, **kwargs):
    ...
    try:
        ...
        if not result:
            return json.dumps({'code':1, 'errmsg':'result is  null'})
        utils.write_log('api').info(username, 'update role success!' )
...
def role_delete(auth_info,**kwargs):
    ...
    try:
        ...
        if not result :
            return json.dumps({'code':1, 'errmsg':'result is null'})
        utils.write_log('api').info(username, 'delete role successed')
